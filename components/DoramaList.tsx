...
import { Play, Heart, Plus, Check, MoreVertical, Edit3, Trash2, Tv, TrendingUp, Calendar } from 'lucide-react';
...
const DoramaList: React.FC<DoramaListProps> = ({ title, doramas, type, onAdd, onUpdate, onDelete, onEdit }) => {
  ...
  const handleQuickAdd = (e: React.MouseEvent, dorama: Dorama) => {
      e.stopPropagation();
      if (onUpdate) {
          const nextEp = (dorama.episodesWatched || 0) + 1;
          if (nextEp <= (dorama.totalEpisodes || 999)) {
                                    ...
                                    <div className="flex justify-between items-end mb-2">
                                        <div>
                                            ...
                                            <p className="text-sm font-black text-gray-800">
                                                Ep. <span className="text-pink-600">{dorama.episodesWatched}</span> <span className="text-gray-300">/</span> {dorama.totalEpisodes}
                                            </p>
                                        </div>
                                        <button 
                                            onClick={(e) => handleQuickAdd(e, dorama)}
                                            className="bg-pink-600 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md shadow-pink-200 active:scale-95 transition-all flex items-center gap-1 hover:bg-pink-700"
                                        >
                                            <Plus className="w-3 h-3" /> 1 Ep
                                        </button>
                                    </div>
                                    {/* Progress Bar */}
